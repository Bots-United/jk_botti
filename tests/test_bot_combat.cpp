...
static int test_mode2_breakable_blocking_path(void)
{
   ...
   // Set up waypoint ahead so the bot traces toward it
   waypoints[0].origin = Vector(100, 0, 0);
   waypoints[0].flags = 0;
   testbot.curr_waypoint_index = 0;
...
static int test_mode2_no_waypoint_uses_forward(void)
{
   printf("BotFindEnemy mode 2 (no waypoint, traces forward):\n");
   mock_reset();
   ...
   setup_bot_for_test(testbot, pBotEdict);
   pBotEdict->v.v_angle = Vector(0, 0, 0); // facing +X
   testbot.curr_waypoint_index = -1; // no waypoint

   // Breakable directly ahead
   edict_t *pBreakable = create_breakable(Vector(50, 0, 0), 50);
   mock_add_breakable(pBreakable, TRUE);
   ...
   TEST("mode 2: no waypoint, forward trace hits breakable");
   BotFindEnemy(testbot);
   ASSERT_PTR_EQ(testbot.pBotEnemy, pBreakable);
   PASS();
...
int main(void)
{
   ...
   printf("\n");
   rc |= test_mode2_no_waypoint_uses_forward();
